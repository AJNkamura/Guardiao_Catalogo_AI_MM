...
from config import SMTP_HOST, SMTP_PORT, SMTP_USER, SMTP_PASSWORD, EMAIL_RECIPIENTS, CANAIS
...
def montar_html(resultados):
    ...
    def linha(r):
        link = f'<a href="{r.get("link","")}">abrir</a>' if r.get("link") else ""
        canal_label = CANAIS.get(r["canal"], {}).get("label", r["canal"])
        return (
            f"<tr><td>{r['id_produto']}</td><td>{r.get('nome_produto','')[:60]}</td>"
            f"<td>{canal_label}</td><td>{r.get('data','')}</td><td>{link}</td>"
    ...
    def tabela(lista, cor):
        ...
        return (
            ...
            f'<tr style="background:{cor};color:#fff"><th>ID Produto</th><th>Nome</th><th>Canal</th>'
            f"<th>Data zerada</th><th>Link</th><th>Observação</th></tr>{linhas}</table>"
        )
